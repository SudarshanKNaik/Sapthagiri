...
function statusBadge(status) {
  if (status === "eligible") return "badge badge-lime";
  if (status === "needs_documents") return "badge bg-amber-100 text-amber-800";
...
export default function EligibilityDashboard({ results = [], selectedIds = [], onToggleSelect, onStartApply }) {
  return (
    <div className="space-y-3">
      {results.map((r) => (
        <div key={r.scheme.id} className="card p-6 text-left">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-2">
              <div className={statusBadge(r.status)}>{r.status.replace("_", " ")}</div>
              {r.status === "eligible" && r.scheme.min_marks >= 90 && (
                <div className="badge badge-dark text-[10px]">TOP MATCH ✨</div>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 flex gap-2">
            <button
              type="button"
              className={`flex-1 btn-secondary ${selectedIds.includes(r.scheme.id) ? "border-[var(--border-dark)] bg-gray-50" : ""}`}
